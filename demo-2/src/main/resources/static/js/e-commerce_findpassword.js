...
$(".login-btn").on("click",function(){
	...
	if(checkCellPhone(cellphone)==false){
		$("#msg").css("display", "block");
        $("#msg").text("手机号输入错误");
	}else if(code != null && code != '' && newpassword1 != null && newpassword1 != '' && newpassword2 != null && newpassword2 != ''){
...
function checkCellPhone(cellphone){
	if(!/(^1\d{10}$)|(^[0-9]\d{7}$)/.test(cellphone)) {
	    return false;
	}else{
		return true;
	}
...
$(".cellphone").blur(function(){
    //$(this).css("background-color","#ffffff");
    if(checkCellPhone($(this).val()) == false){
    	$(this).siblings(".msg").text("手机号格式错误");
    }else{

        ...
        $.ajax({
            ...
            success : function(data) {
                ...
                for(var i=0;i<data.size;i++){
                    ...
                    optionString1+='    <td>\n' +
                        '    <a href="./service_edit.html" onclick="edit(\''+data.list[i].id+'\')"><span class="handle-btn"><i class="fa fa-edit fa-fw"></i>编辑</span></a>\n' +
                        '    <a href="javascript:void(0);" onclick="productDel(\''+data.list[i].id+'\')"><span class="handle-btn"><i class="fa fa-close fa-fw"></i>删除</span></a>\n' ;

                    if (data.list[i].status!=0){
                        optionString1+='    <a href="javascript:void(0);" onclick="offline(\''+data.list[i].id+'\')"><span  class="handle-btn"><i class="fa fa-arrow-down fa-fw"></i>下线</span></a>\n';
                    }else {
                        optionString1+='    <a href="javascript:void(0);" onclick="online(\''+data.list[i].id+'\')"><span class="handle-btn"><i class="fa fa-arrow-up fa-fw"></i>上线</span></a>\n';
                    }
                    optionString1+= '</td></tr>';
                }
                var optionString2 = "";
                for (var i=1;i<data.pagesum+1;i++){
                    ...
                    if(i<data.pageNum+3&&i>data.pageNum-3){

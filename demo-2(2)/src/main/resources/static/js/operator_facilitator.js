...
$(function(){
	...
	if(url.indexOf("?")!=-1){                     //判断是否存在参数
		...
		if(cpage<=10){
			for(var i=1;i<=cpage;i++){
			...
			}}else if(cpage>10 && 1<=nowpage && nowpage<=6){		// pagestart=20
																	// nowpage=11
																	// cpage=15
				...
				for(var i=1;i<=10;i++){
					if(i==nowpage){
						...
						// strhtml+='<a href=/userAll?pageStart='+psize*(i-1)+'
						// style="background-color:#aaaaaa"><div
						// class="nowpage">'+i+'</div></a>';
					}else{
						strhtml+='<span> <a href=/fuwushang?pageStart='+psize*(i-1)+'&service_name='+service_name+'>'+i+'</a></span>';
					}
...
function goto(id){
	$.ajax({
		...
		success:function(data){
			...
			$(".serviceName").text(data.service.serviceName);
			$(".regionName").text(data.service.regionName);
			$(".cellphone").text(data.service.cellphone);
			$(".serviceInfo").text(data.service.serviceInfo);
		},
		error:function(){
			alert("error");
		}
